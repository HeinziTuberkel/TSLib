...
type
	...
	private
		...
		procedure WaitForActivated(Data: PtrInt);
		procedure WaitForShown(Data: PtrInt);
	protected
...
begin
	...
	if Assigned(fAfterShow) then
  	Application.QueueAsyncCall(@WaitForShown, 0);
end;
...
//***************************************************************************************
procedure TTSForm.WaitForShown(Data: PtrInt);
begin
	if Assigned(fAfterShow) then
	begin
		Refresh;
...
begin
	...
	if Assigned(fAfterActivate) then
		Application.QueueAsyncCall(@WaitForActivated, 0);
end;

//***************************************************************************************
procedure TTSForm.WaitForActivated(Data: PtrInt);
begin
	if Assigned(fAfterActivate) then
		fAfterActivate(Self);
end;
